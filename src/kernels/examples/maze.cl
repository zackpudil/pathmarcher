static float tmax() { return 50.0f; }
...
static float3 key(float ti) {
  return (float3)(0.0f, 0.6f, ti + 1.0f);
}
...
static float2 de(float3 p, float time) {
  ...
  return (float2)(min(d, p.y + 1.0f), 0.0f);
}

static float3 shade(float3 pos, float3 nor, float3 ref, float3 rd, float2 t, float sa, float time) {
  ...
  col += clamp(dot(lig, nor), 0.0f, 1.0f)
    *step(0.0f, -trace(pos + nor*0.001f, lig, dis, time).x);

  return col;
}

static void camera(float3* ro, float3* rd, float2 uv, float time) {
  *ro = (float3)(0.0f, 1.0f, time);
  float3 ww = normalize((float3)(0.0f, 0.5f, time + 1.0f)-*ro);
